...
public:
    ...
    // last N movements for a stock (descending by day/seg)
...
// ----- News loaders -----
map<int,string> loadGlobalNews(const string &dataset) {
    map<int,string> out;
...
int main(int argc, char** argv) {
    ...
    while (running) {
        ...
        if (curSeg == "M" && globalNews.count(curDay)) {
            cout << "=== Morning News ===\n";
            printWrapped(globalNews[curDay], 80, "");
            cout << "\n";
        }
        ...
        if (ch == 1) {
            ...
            cout << "Enter stock id: "; int id; cin >> id;
            cout << "1) Last 5 segments\n2) Last 3 days analysis\nChoose: ";
            int subCh; cin >> subCh;

            logger.appendView(run_id, curDay, curSeg, now_iso(), id, to_string(id));
            const StockRow* maybe = market.findById(id);
            if (maybe) {
                cout << "Stock: " << maybe->symbol << " - " << maybe->company << " (" << maybe->sector << ")\n";
                cout << "Short: ";
            ...
            {
                double curP=0, prevC=0, high=0, low=0;
                ...
                if (prevSegInfo.first >= 1) {
                    auto prev = market.getPriceExact(prevSegInfo.first, prevSegInfo.second, id);
                    if (prev.first) prevC = prev.second.close;
                }
                cout << "PrevClose: ";
                if (prevC) cout << fixed << setprecision(2) << prevC; else cout << "-";
                cout << "  CurPrice: " << fixed << setprecision(2) << curP
                     << "  DayHigh: " << fixed << setprecision(2) << high
                     << "  DayLow: " << fixed << setprecision(2) << low << "\n";

                if (subCh == 1) {
                    auto hist = market.getLastN(id, 5);
                    if (hist.empty()) cout << "  (no history found)\n";
                    else {
                        cout << "Last 5 segments (day seg close change% vol):\n";
                        int ups = 0;
                        for (auto &h : hist) {
                            cout << "  Day " << h.day << " " << h.seg
                                 << "  " << fixed << setprecision(2) << h.close
                                 << "  " << fixed << setprecision(2) << h.change << "%  vol=" << h.volume << "\n";
                            if (h.change > 0.0) ups++;
                        }
                        cout << "Trend: " << ups << "/5 up segments\n";
                    }
                } else if (subCh == 2) {
                    cout << "Last 3 Days Analysis:\n";
                    for (int d = curDay; d >= curDay - 2 && d >= 1; --d) {
                        bool found = false;
                        PriceInfo pi;
                        for (int si = (int)segments.size()-1; si >= 0; --si) {
                            auto pp2 = market.getPriceExact(d, segments[si], id);
                            if (pp2.first) { pi = pp2.second; found = true; break; }
                        }
                        if (found) {
                            cout << "  Day " << d << ": Close="
                                 << fixed << setprecision(2) << pi.close
                                 << " (Seg " << pi.seg << ")\n";
                        } else {
                            cout << "  Day " << d << ": No data\n";
                        }
